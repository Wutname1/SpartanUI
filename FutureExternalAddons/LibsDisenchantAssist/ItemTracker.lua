...
---@class ItemTracker
LibsDisenchantAssist.ItemTracker = {}
local ItemTracker = LibsDisenchantAssist.ItemTracker
    ...
    for bag = 0, 4 do
        ...
        if numSlots then
            for slot = 1, numSlots do
                ...
                if itemID and not LibsDisenchantAssistDB.itemFirstSeen[itemID] then
                    self:RecordFirstSeen(itemID, currentTime)
                end
            end
        end
...
function ItemTracker:RecordFirstSeen(itemID, timestamp)
    if not LibsDisenchantAssistDB.itemFirstSeen[itemID] then
        LibsDisenchantAssistDB.itemFirstSeen[itemID] = timestamp
    end
end
...
function ItemTracker:GetItemFirstSeenDate(itemID)
    local timestamp = LibsDisenchantAssistDB.itemFirstSeen[itemID]
    if timestamp then
