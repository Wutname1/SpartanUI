...
LibsDataBar.pluginManager = LibsDataBar.pluginManager or setmetatable({
    managementFrame = nil,
    pluginList = {},
    filterSettings = {
        showEnabled = true,
    ...
    if LibsDataBar.ldb and LibsDataBar.ldb.registeredObjects then
        for ldbName, wrapper in pairs(LibsDataBar.ldb.registeredObjects) do
            if wrapper and wrapper.plugin then
                self:AddPluginToList(wrapper.plugin.id, wrapper.plugin, "ldb")
            end
        end
    end
...
function PluginManager:AddPluginToList(pluginId, plugin, type)
    ...
    for barId, bar in pairs(LibsDataBar.bars or {}) do
        if bar.plugins and bar.plugins[pluginId] then
            self.pluginList[pluginId].status.bars[barId] = true
        end
    end
end
...
function PluginManager:StartPerformanceMonitoring()
    ...
    if not self.refreshTimer then
        self.refreshTimer = C_Timer.NewTicker(5, function()
...
function PluginManager:CalculateEfficiency(pluginInfo)
    ...
    if perf.updateTime > 1 then
        score = score - math.min(perf.updateTime * 10, 30)
    end
    
    -- Penalize high memory usage (> 1KB per call is concerning)
    if perf.memoryUsage > 1 then
        score = score - math.min(perf.memoryUsage * 5, 20)
    end
...
function PluginManager:UpdatePluginMetrics()
    for pluginId, pluginInfo in pairs(self.pluginList) do
        ...
        for barId, bar in pairs(LibsDataBar.bars or {}) do
            if bar.plugins and bar.plugins[pluginId] then
                pluginInfo.status.bars[barId] = true
            end
        end
...
function PluginManager:RefreshPluginList()
    ...
    -- Update status text
    if self.managementFrame then
        self.managementFrame:SetStatusText(string.format("%d of %d plugins shown", #sortedPlugins, self:GetPluginCount()))
    end
end
...
function PluginManager:GetFilteredPlugins()
    ...
    for pluginId, pluginInfo in pairs(self.pluginList) do
        ...
        -- Type filter
        if not ((pluginInfo.type == "native" and self.filterSettings.showNative) or
                (pluginInfo.type == "ldb" and self.filterSettings.showLDB)) then
            passesFilter = false
        end
        
        -- Status filter
        if not ((pluginInfo.status.enabled and self.filterSettings.showEnabled) or
                (not pluginInfo.status.enabled and self.filterSettings.showDisabled)) then
            passesFilter = false
        end
        ...
        if self.filterSettings.category ~= "all" then
            ...
            if pluginInfo.type == "ldb" then
                pluginCategory = "LDB"
            end
            if pluginCategory ~= self.filterSettings.category then
                passesFilter = false
            end
        end
        ...
        if self.filterSettings.searchText and self.filterSettings.searchText ~= "" then
            ...
            if not (pluginName:find(searchText) or pluginDesc:find(searchText) or pluginId:lower():find(searchText)) then
                passesFilter = false
            end
        end
        
        if passesFilter then
            table.insert(filtered, pluginInfo)
        end
    end
...
function PluginManager:CreatePluginWidget(pluginInfo)
    ...
    perfLabel:SetText(string.format("%sEff: %d%%|r\n|cff888888%.2fms, %.1fKB|r", 
                                   perfColor, perf.efficiency,
                                   perf.updateTime, perf.memoryUsage))
    ...
    reloadBtn:SetCallback("OnClick", function()
        if LibsDataBar.hotReload then
            LibsDataBar.hotReload:ReloadPlugin(plugin.name)
        end
    end)
...
function PluginManager:ShowPerformanceReport()
    ...
    table.sort(sortedPlugins, function(a, b) return a.perf.efficiency > b.perf.efficiency end)
