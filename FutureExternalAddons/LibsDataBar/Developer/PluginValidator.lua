...
local VALIDATION_RULES = {
    requiredFields = function(plugin)
        ...
        for _, field in ipairs(required) do
            if not plugin[field] or plugin[field] == "" then
    ...
    validId = function(plugin)
        ...
        -- Check ID format
        if not plugin.id:match("^[%w_]+$") then
            return false, "ID contains invalid characters (use only letters, numbers, underscore)"
        end
        
        -- Check ID prefix
        if not plugin.id:match("^LibsDataBar_") and plugin.type ~= "ldb" then
            return false, "Native plugin ID should start with 'LibsDataBar_'"
        end
    ...
    validVersion = function(plugin)
        ...
        if not plugin.version:match("^%d+%.%d+%.%d+") then
            return false, "Version should follow semantic versioning (x.y.z)"
        end
...
local QUALITY_CHECKS = {
    documentation = function(plugin)
        ...
        -- Author information
        if plugin.author and plugin.author ~= "" then
            score = score + 5
        end
        
        -- Category specified
        if plugin.category and plugin.category ~= "" then
            score = score + 5
        end
        
        -- Version information
        if plugin.version and plugin.version:match("^%d+%.%d+%.%d+") then
            score = score + 5
        end
        
        -- Dependencies documented
        if plugin.dependencies and next(plugin.dependencies) then
            score = score + 5
        end
    ...
    functionality = function(plugin)
        ...
        for _, method in ipairs(optionalMethods) do
            if plugin[method] and type(plugin[method]) == "function" then
                score = score + 5
            end
        end
...
function PluginValidator:Initialize()
    ...
    LibsDataBar:DebugLog("info", "PluginValidator initialized with " .. 
                        self:GetRuleCount() .. " rules and " .. 
                        self:GetCheckCount() .. " quality checks")
end
...
function PluginValidator:ValidatePlugin(plugin)
    ...
    -- Store score
    if plugin.id then
        self.scores[plugin.id] = result.score
    end
...
function PluginValidator:GenerateSuggestions(result, plugin)
    ...
    if result.breakdown.functionality and result.breakdown.functionality.score < 20 then
        if not plugin.GetIcon then
            table.insert(result.suggestions, "Add GetIcon() method for visual appeal")
        end
        if not plugin.UpdateTooltip then
            table.insert(result.suggestions, "Add UpdateTooltip() method for better user experience")
        end
        if not plugin.OnClick then
            table.insert(result.suggestions, "Add OnClick() method for interactivity")
        end
        if not plugin.GetConfigOptions then
            table.insert(result.suggestions, "Add GetConfigOptions() method for user customization")
        end
    end
...
function PluginValidator:GetValidationReport(plugin)
    ...
    for category, breakdown in pairs(result.breakdown) do
        ...
        table.insert(report, string.format("  %s: %d/%d (%d%%)", 
                                          category:gsub("^%l", string.upper), 
                                          breakdown.score, breakdown.max, percentage))
    end
    table.insert(report, "")
...
if LibsDataBar.validator then
    LibsDataBar.validator:Initialize()
end
