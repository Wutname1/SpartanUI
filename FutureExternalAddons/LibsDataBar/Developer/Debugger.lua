...
LibsDataBar.debugger = LibsDataBar.debugger
	or setmetatable({
		enabled = false,
		categories = {
...
function Debugger:Log(level, message, category, source)
	...
	if level == 'error' or level == 'critical' then entry.stackTrace = debugstack(2) end
...
function Debugger:GetRecentLogs(count, category, level)
	...
	for i = #self.logHistory, 1, -1 do
		...
		-- Apply filters
		if (not category or entry.category == category) and (not level or entry.level == level) then
...
function Debugger:GetPerformanceStats()
	...
	for _, entry in ipairs(self.logHistory) do
		if entry.timestamp >= recentTime then stats.recentActivity = stats.recentActivity + 1 end
	end
...
function Debugger:GenerateReport()
	...
	if #recentErrors > 0 then
		...
		for _, entry in ipairs(recentErrors) do
			table.insert(report, string.format('  [%s] %s: %s', date('%H:%M:%S', entry.timestamp), entry.category:upper(), entry.message))
		end
		table.insert(report, '')
	end
	...
	if LibsDataBar.plugins then
		...
		for pluginId, plugin in pairs(LibsDataBar.plugins) do
			table.insert(report, string.format('  %s: %s (v%s)', plugin.name or pluginId, plugin.enabled and 'ENABLED' or 'DISABLED', plugin.version or 'unknown'))
		end
		table.insert(report, '')
	end
...
function Debugger:Profile(name, func, ...)
	if not self.enabled or not self.categories['performance'] then return func(...) end
...
function Debugger:DumpObject(obj, name, maxDepth)
	...
	local function dump(o, depth, visited)
		...
		elseif objType == 'table' then
			if depth == 0 then visited[o] = true end
