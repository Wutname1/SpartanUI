...
function DataBar:Create(config)
	...
	if LibsDataBar.themes then LibsDataBar.themes:ApplyThemeToBar(bar) end
	...
	-- Fire creation event
	if LibsDataBar.callbacks then LibsDataBar.callbacks:Fire('LibsDataBar_BarCreated', config.id, bar) end
...
function DataBar:ApplyDefaults()
	...
	for key, defaultValue in pairs(defaults) do
		if self.config[key] == nil then self.config[key] = defaultValue end
	end
...
function DataBar:UpdateSize()
	...
	if width == 0 then width = UIParent:GetWidth() end

	self.frame:SetSize(width, height)
	self.frame:SetScale(scale)

	-- Fire size change event through API
	if LibsDataBar.API then LibsDataBar.API:NotifyPositionChange(self.id, 'resize') end
end
...
function DataBar:UpdatePosition()
	...
	if self.config.behavior.strata then self.frame:SetFrameStrata(self.config.behavior.strata) end

	if self.config.behavior.level then self.frame:SetFrameLevel(self.config.behavior.level) end

	-- Fire position change event through API
	if LibsDataBar.API then LibsDataBar.API:NotifyPositionChange(self.id, 'move') end
end
...
function DataBar:UpdateAppearance()
	...
	if not appearance.background or not appearance.background.show then
		...
		if self.backgroundTexture then self.backgroundTexture:Hide() end
		if self.borderTexture then self.borderTexture:Hide() end
		return
	end
	...
	if self.frame.SetBackdrop and not self.useManualBackground then
		...
		if bg.color then self.frame:SetBackdropColor(bg.color.r, bg.color.g, bg.color.b, bg.color.a) end
		...
		if not self.backgroundTexture then self.backgroundTexture = self.frame:CreateTexture(nil, 'BACKGROUND') end

		self.backgroundTexture:SetTexture(bg.texture or 'Interface\\Tooltips\\UI-Tooltip-Background')
		if bg.color then self.backgroundTexture:SetVertexColor(bg.color.r, bg.color.g, bg.color.b, bg.color.a) end
		...
		if appearance.border and appearance.border.show then
			if not self.borderTexture then self.borderTexture = self.frame:CreateTexture(nil, 'BORDER') end
...
function DataBar:SetupMouseHandling()
	...
	self.frame:SetScript('OnDragStart', function(frame)
		if not InCombatLockdown() then self:StartDragging() end
	end)

	self.frame:SetScript('OnDragStop', function(frame)
		if not InCombatLockdown() then self:StopDragging() end
	end)
...
function DataBar:RegisterEvents()
	...
	LibsDataBar.events:RegisterEvent('PLAYER_REGEN_DISABLED', function()
		if self.config.behavior.combatHide then self:Hide() end
	end, { owner = self.id })

	LibsDataBar.events:RegisterEvent('PLAYER_REGEN_ENABLED', function()
		if self.config.behavior.combatHide then self:Show() end
	end, { owner = self.id })
end
...
function DataBar:AddPlugin(plugin)
	...
	if LibsDataBar.themes and button then
		...
		if currentTheme then LibsDataBar.themes:ApplyThemeToButton(button, currentTheme.button) end
	end
	...
	-- Fire event
	if LibsDataBar.callbacks then LibsDataBar.callbacks:Fire('LibsDataBar_PluginAdded', self.id, plugin.id) end
...
function DataBar:RemovePlugin(pluginId)
	...
	-- Fire event
	if LibsDataBar.callbacks then LibsDataBar.callbacks:Fire('LibsDataBar_PluginRemoved', self.id, pluginId) end
...
function DataBar:UpdateLayout()
	...
	for _, pluginId in ipairs(self.pluginOrder) do
		local button = self.plugins[pluginId]
		if button and button.frame and button.frame:IsShown() then tinsert(sortedPlugins, button) end
	end
...
function DataBar:CreatePluginButton(plugin)
	...
	if button then LibsDataBar:DebugLog('debug', 'Created PluginButton for plugin: ' .. plugin.id) end
...
function DataBar:UpdatePlugins()
	for pluginId, button in pairs(self.plugins) do
		...
		if button.plugin and button.plugin.GetText and button.frame and button.frame.SetText then
			...
			if newText and button.frame:GetText() ~= newText then button.frame:SetText(newText) end
		end
	end
end
...
function DataBar:Show()
	...
	if LibsDataBar.callbacks then LibsDataBar.callbacks:Fire('LibsDataBar_BarShown', self.id) end
end
...
function DataBar:Hide()
	...
	if LibsDataBar.callbacks then LibsDataBar.callbacks:Fire('LibsDataBar_BarHidden', self.id) end
end
...
function DataBar:StartAutoHideTimer()
	...
	if self.autoHideTimer then self.autoHideTimer:Cancel() end
...
function DataBar:StartDragging()
	...
	if LibsDataBar.callbacks then LibsDataBar.callbacks:Fire('LibsDataBar_BarDragStart', self.id, self) end
...
function DataBar:StopDragging()
	...
	if LibsDataBar.UpdateBarPositions then LibsDataBar:UpdateBarPositions() end

	-- Fire drag stop event
	if LibsDataBar.callbacks then LibsDataBar.callbacks:Fire('LibsDataBar_BarDragStop', self.id, self, newPosition) end
...
function DataBar:Destroy()
	...
	-- Fire event
	if LibsDataBar.callbacks then LibsDataBar.callbacks:Fire('LibsDataBar_BarDestroyed', self.id) end
