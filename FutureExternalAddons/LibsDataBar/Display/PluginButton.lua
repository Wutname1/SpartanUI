...
function PluginButton:CreateFrame()
    ...
    -- Register for clicks
    if self.config.interactions.enableClick then
...
function PluginButton:CreateIcon()
    ...
    -- Set icon from plugin
    if self.plugin.icon then
        self.icon:SetTexture(self.plugin.icon)
    end
...
function PluginButton:SetupInteractions()
    ...
    -- Click handling
    if interactions.enableClick then
...
function PluginButton:CalculateAutoWidth()
    ...
    if self.icon and self.config.icon.position ~= "NONE" then
        width = width + self.config.icon.size + self.config.icon.spacing
    end
...
function PluginButton:UpdateLayout()
    ...
    -- Position icon and text based on icon position
    if self.icon and iconPosition ~= "NONE" then
...
function PluginButton:Update()
    ...
    if self.plugin.GetText and type(self.plugin.GetText) == "function" then
        ...
        if success and newText then
            ...
            if not success then
                LibsDataBar:DebugLog("error", "Plugin " .. self.plugin.id .. " GetText error: " .. tostring(newText))
            end
        end
    end
    
    -- Update icon from plugin if it changed
    if self.plugin.icon and self.icon then
        self.icon:SetTexture(self.plugin.icon)
    end
    ...
    if self.bar and self.bar.OnPluginUpdated then
        self.bar:OnPluginUpdated(self)
    end
end
...
function PluginButton:OnEnter()
    ...
    -- Show tooltip
    if self.config.interactions.enableTooltip then
        self:ShowTooltip()
    end
    
    -- Fire bar event
    if self.bar.callbacks then
        self.bar.callbacks:Fire("LibsDataBar_ButtonEnter", self.plugin.id, self)
    end
end
...
function PluginButton:OnLeave()
    ...
    -- Fire bar event
    if self.bar.callbacks then
        self.bar.callbacks:Fire("LibsDataBar_ButtonLeave", self.plugin.id, self)
    end
end
...
function PluginButton:OnClick(mouseButton, down)
    ...
    if self.plugin.OnClick and type(self.plugin.OnClick) == "function" then
        ...
        if not success then
            LibsDataBar:DebugLog("error", "Plugin " .. self.plugin.id .. " OnClick error: " .. tostring(result))
        end
    end
    
    -- Fire bar event
    if self.bar.callbacks then
        self.bar.callbacks:Fire("LibsDataBar_ButtonClick", self.plugin.id, mouseButton, self)
    end
...
function PluginButton:OnMouseWheel(delta)
    ...
    if self.plugin.OnMouseWheel and type(self.plugin.OnMouseWheel) == "function" then
        ...
        if not success then
            LibsDataBar:DebugLog("error", "Plugin " .. self.plugin.id .. " OnMouseWheel error: " .. tostring(result))
        end
    end
end
...
function PluginButton:OnDragStop()
    if self.bar and self.bar.frame then
        self.bar.frame:StopMovingOrSizing()
    end
end
...
function PluginButton:ShowTooltip()
    ...
    if self.plugin.UpdateTooltip and type(self.plugin.UpdateTooltip) == "function" then
        ...
        if self.plugin.description then
            GameTooltip:AddLine(self.plugin.description, 1, 1, 1, true)
        end
    end
...
function PluginButton:Show()
    ...
    if self.config.animations.enable and self.fadeIn then
        self.fadeIn:Play()
    end
end
