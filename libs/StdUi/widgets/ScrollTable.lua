...
local methods = {
	...
	ScrollToLine = function(self, line)
		...
		self:DoVerticalScroll(self.rowHeight * (line - 1), self.rowHeight, function(s)
			s:Refresh()
		end)
	end,
	...
	SetDisplayRows = function(self, numberOfRows, rowHeight)
		...
		for i = 1, numberOfRows do
			...
			if not rowFrame.columns then rowFrame.columns = {} end
						...
						for event, handler in pairs(self.cellEvents) do
							cell:SetScript(event, function(cellFrame, ...)
								if table.offset then
									...
									table:FireCellEvent(event, handler, cellFrame, rowFrame, rowData, columnData, rowIndex, ...)
								end
							end)
						end
					end
	...
	SortData = function(self, sortBy)
		-- sanity check
		if not self.sortTable or (#self.sortTable ~= #self.data) then self.sortTable = {} end
		...
		if not sortBy then
			...
			while i <= #self.columns and not sortBy do
				if self.columns[i].sort then sortBy = i end
				i = i + 1
			end
		end
	...
	SetFilter = function(self, filter, noSort)
		self.Filter = filter
		if not noSort then self:SortData() end
	end,
	...
	DoFilter = function(self)
		...
		for row = 1, #self.data do
			...
			if self:Filter(rowData) then TableInsert(result, realRow) end
		end
	...
	DoCellUpdate = function(table, shouldShow, rowFrame, cellFrame, value, columnData, rowData, rowIndex)
		if shouldShow then
			...
			if type(color) == 'function' then color = color(table, value, rowData, columnData) end
	...
	FireCellEvent = function(self, event, handler, ...)
		if not handler(self, ...) then
			if self.cellEvents[event] then self.cellEvents[event](self, ...) end
		end
	end,

	FireHeaderEvent = function(self, event, handler, ...)
		if not handler(self, ...) then
			if self.headerEvents[event] then self.headerEvents[event](self, ...) end
		end
	end,
...
local cellEvents = {
	...
	OnLeave = function(table, cellFrame, rowFrame, rowData, columnData, rowIndex)
		if rowIndex ~= table.selected or not table.selectionEnabled then table:SetHighLightColor(rowFrame, nil) end
	...
	OnClick = function(table, columnFrame, columnHeadFrame, columnIndex, button, ...)
		if button == 'LeftButton' then
			...
			for i, _ in ipairs(columnHeadFrame.columns) do
				if i ~= columnIndex then columns[i].sort = nil end
			end
