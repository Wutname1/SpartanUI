...
function StdUi:Slider(parent, width, height, value, vertical, min, max)
	...
	slider.ThumbTexture:SetVertexColor(self.config.backdrop.slider.r, self.config.backdrop.slider.g, self.config.backdrop.slider.b, self.config.backdrop.slider.a)
...
local SliderWithBoxMethods = {
	SetValue = function(self, v)
		...
		if self.OnValueChanged then self.OnValueChanged(self, v) end
	end,
...
function StdUi:SliderWithBox(parent, width, height, value, min, max)
	...
	if min and max then widget:SetMinMaxValues(min, max) end
