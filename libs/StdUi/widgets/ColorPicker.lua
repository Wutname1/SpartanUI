...
local ColorPickerMethods = {
	...
	SetColorAlpha = function(self, a, fromSlider)
		...
		if not fromSlider then self.alphaSlider:SetValue(100 - a * 100) end
...
function StdUi:ColorPicker(parent, alphaSliderTexture)
	...
	cpf.alphaThumbTexture = self:Texture(cpf.alphaSlider, barWidth, thumbWidth, [[Interface\Buttons\UI-ColorPicker-Buttons]])
...
local ColorPickerFrameOkCallback = function(self)
	local cpf = self:GetParent()
	if cpf.okCallback then cpf.okCallback(cpf) end
...
local ColorPickerFrameCancelCallback = function(self)
	local cpf = self:GetParent()
	if cpf.cancelCallback then cpf.cancelCallback(cpf) end
...
local ColorInputMethods = {
	SetColor = function(self, c)
		...
		if self.OnValueChanged then self:OnValueChanged(c) end
	end,
...
local ColorInputEvents = {
	OnClick = function(self)
		self.stdUi:ColorPickerFrame(self.color.r, self.color.g, self.color.b, self.color.a, function(cpf)
