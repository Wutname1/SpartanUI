...
function module:StatusBars()
	...
	local showRepTooltip = function(self)
		...
		if name then
			...
			Fel_StatusBarTooltipHeader:SetText(format("%s ( %s / %s ) %d%% %s", name, SUI:comma_value(current-low), SUI:comma_value(high-low), ratio*100,_G["FACTION_STANDING_LABEL"..react]));
	...
	local showAPTooltip = function(self)
		...
		if HasArtifactEquipped() and not C_ArtifactUI.IsEquippedArtifactMaxed() then
			...
			Fel_StatusBarTooltipText:SetFormattedText("( %s / %s ) %d%%", SUI:comma_value(xp), SUI:comma_value(xpForNextPoint), ratio*100)
		else
	...
	local showAzeriteTooltip = function(self)
		if C_AzeriteItem.HasActiveAzeriteItem() then
			...
			local xpToNextLevel = totalLevelXP - xp;
			local ratio = (xp / totalLevelXP)
			Fel_StatusBarTooltipHeader:SetText(ARTIFACT_POWER_TOOLTIP_TITLE:format(BreakUpLargeNumbers(arfifactTickParent.totalXP, true), BreakUpLargeNumbers(arfifactTickParent.xp, true), BreakUpLargeNumbers(arfifactTickParent.xpForNextPoint, true)), HIGHLIGHT_FONT_COLOR:GetRGB());
			Fel_StatusBarTooltipText:SetText(ARTIFACT_POWER_TOOLTIP_BODY:format(arfifactTickParent.numPointsAvailableToSpend), nil, nil, nil, true);
		end
		tooltip:Show();
	end
	...
	Fel_StatusBar_Right:SetScript("OnEnter",function(self)
		...
		if SUI.DB.StatusBars.right == "az" and SUI.DB.StatusBars.AzeriteBar.ToolTip == "click" then showAzeriteTooltip(self); end
	end);
...
function module:UpdateStatusBars()
	if SUI.DB.StatusBars.left ~= "disabled" then Fel_StatusBar_Left:Show(); updateText(Fel_StatusBar_Left, "left") else Fel_StatusBar_Left:Hide(); end
	if SUI.DB.StatusBars.right ~= "disabled" then Fel_StatusBar_Right:Show(); updateText(Fel_StatusBar_Right, "left") else Fel_StatusBar_Right:Hide(); end
end
