...
function module:InitArtwork()
	Artwork_Core:ActionBarPlates('War_ActionBarPlate')
...
function module:SlidingTrays()
	local bg, bgCollapsed, btnDown, btnUp
	...
	for _, key in ipairs(trayIDs) do
		...
		bg = tray:CreateTexture(nil, 'BACKGROUND')
		...
		bgCollapsed = tray:CreateTexture(nil, 'BACKGROUND')
		...
		btnUp = CreateFrame('BUTTON')
		local UpTex = tray:CreateTexture()
		...
		btnUp:SetScript(
			'OnClick',
			function()
				SUI.DB.Styles.War.SlidingTrays[key].collapsed = true
				tray.bg:Hide()
				tray.btnUp:Hide()
				tray.bgCollapsed:Show()
				tray.btnDown:Show()
			end
		)
		btnUp:SetPoint('BOTTOM', tray, 'BOTTOM', 1, 1)

		btnDown = CreateFrame('BUTTON')
		local DownTex = tray:CreateTexture()
		...
		btnDown:SetScript(
			'OnClick',
			function()
				SUI.DB.Styles.War.SlidingTrays[key].collapsed = false
				tray.bg:Show()
				tray.btnUp:Show()
		...
		btnDown:SetPoint('TOP', tray, 'TOP', 1, -4)

		tray.bg = bg
		tray.btnUp = btnUp

		tray.bgCollapsed = bgCollapsed
		tray.btnDown = btnDown
