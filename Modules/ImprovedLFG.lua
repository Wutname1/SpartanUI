...
local function DetailRoles(self, searchResultInfo)
	...
	if activityInfo.displayType ~= Enum.LFGListDisplayType.RoleEnumerate or IsPending(self.resultID) then return end
...
local function LeaderRating(self, searchResultInfo)
	...
	if IsPending(self.resultID) then return end
	...
	if searchResultInfo.voiceChat and searchResultInfo.voiceChat ~= '' then textWidth = textWidth - 20 end
