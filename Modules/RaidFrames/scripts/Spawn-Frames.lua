...
function RaidFrames:UpdateRaid(event,...)
	if SUI.RaidFrames == nil then return end
	...
	elseif SUI.RaidFrames:IsShown() then
		...
		if SUI.RaidFrames.HideTmp then SUI.RaidFrames.Hide = SUI.RaidFrames.HideTmp end
...
function RaidFrames:OnEnable()
	...
	SUI.RaidFrames:SetScript("OnMouseDown",function(self,button)
	...
	SUI.RaidFrames:SetScript("OnMouseUp",function(self,button)
	...
	SUI.RaidFrames.mover:SetScript("OnEvent",function()
		RaidFrames.locked = 1;
		SUI.RaidFrames.mover:Hide();
	end);
