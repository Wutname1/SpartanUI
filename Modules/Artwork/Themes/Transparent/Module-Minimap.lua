...
function Transparent_updateButtons()
...
function Transparent_modifyMinimapLayout()
	frame = CreateFrame("Frame","Transparent_Minimap",Transparent_SpartanUI);
	...
	Minimap:SetPoint("CENTER","Transparent_Minimap","CENTER",0,0);
...
--	-- Create new textures
	...
	MiniMapWorldMapButton:SetNormalTexture("Interface\\AddOns\\SpartanUI_Artwork\\Themes\\Transparent\\Images\\WorldMap-Icon.png")
	MiniMapWorldMapButton:SetPushedTexture("Interface\\AddOns\\SpartanUI_Artwork\\Themes\\Transparent\\Images\\WorldMap-Icon-Pushed.png")
	...
	Minimap.overlay:SetTexture("Interface\\AddOns\\SpartanUI_Artwork\\Themes\\Transparent\\Images\\map-overlay");
...
function Transparent_createMinimapCoords()
	local map = CreateFrame("Frame",nil,Transparent_SpartanUI);
...
function Transparent_MiniMapCreate()
	hooksecurefunc(Minimap,"SetPoint",function(self,input1,input2,input3,input4,input5) -- Check for Changes
		...
		if (Minimap_Conflict_msg) then
			if (self:GetParent():GetName() ~= 'Transparent_Minimap' or relativeTo ~= 'Transparent_Minimap') then
				spartan:Print('|cffff0000SetPoint/SetParent was used on Minimap, potential conflict.|r')
				Minimap_Conflict_msg = false
			end
		end
	end);
	Transparent_modifyMinimapLayout();
	Transparent_createMinimapCoords();
	QueueStatusFrame:ClearAllPoints();
	QueueStatusFrame:SetPoint("BOTTOM",Transparent_SpartanUI,"TOP",0,100);
	module.handleBuff = true
end

function Transparent_ChatBox()
