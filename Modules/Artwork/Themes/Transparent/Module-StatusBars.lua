...
local module = spartan:GetModule("Artwork_Transparent");
----------------------------------------------------------------------------------------------------
...
function module:SetRepColors()
	...
	if DB.RepBar.AutoDefined == true then
		...
		Transparent_ReputationBarFill:SetVertexColor	(color.r, color.g, color.b, 0.8);
		Transparent_ReputationBarFillGlow:SetVertexColor(color.r, color.g, color.b, 0.2);
	else
		r = DB.RepBar.GainedRed
		b = DB.RepBar.GainedBlue
		g = DB.RepBar.GainedGreen
		a = DB.RepBar.GainedBrightness
		Transparent_ReputationBarFill:SetVertexColor	(r, g, b, a);
		Transparent_ReputationBarFillGlow:SetVertexColor(r, g, b, a);
	end
...
function module:EnableStatusBars()
	...
	do -- reputation bar
		repframe = CreateFrame("Frame","Transparent_ReputationBar",Transparent_SpartanUI,"Transparent_StatusBars_RepTemplate");
		repframe:SetPoint("BOTTOMLEFT",Transparent_SpartanUI,"BOTTOM",100,0);
		...
		repframe:SetScript("OnEvent",function()
			...
			Transparent_StatusBarTooltipHeader:SetText(name);
			if ratio == 0 then
				Transpsarent_ReputationBarFill:SetWidth(0.1);
			else
				Transparent_ReputationBarFill:SetWidth(ratio*400);
				module:SetRepColors()
			end
		...
		local showRepTooltip = function()
			tooltip:ClearAllPoints();
			tooltip:SetPoint("BOTTOM",Transparent_ReputationBar,"TOP",-2,-1);
			local name,react,low,high,current,text,ratio = GetWatchedFactionInfo();
			if name then
				text = GetFactionDetails(name);
				ratio = (current-low)/(high-low);
				Transparent_StatusBarTooltipHeader:SetText(format("%s ( %s / %s ) %d%% %s", name, spartan:comma_value(current-low), spartan:comma_value(high-low), ratio*100,_G["FACTION_STANDING_LABEL"..react]));
				Transparent_StatusBarTooltipText:SetText("|cffffd200"..text.."|r");
			else
				Transparent_StatusBarTooltipHeader:SetText(REPUTATION);
				Transparent_StatusBarTooltipText:SetText(REPUTATION_STANDING_DESCRIPTION);
			end
			tooltip:Show();
		end
