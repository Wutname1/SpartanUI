...
----------------------------------------------------------------------------------------------------
if SUI.DB.Styles.Classic.BuffLoc == nil then SUI.DB.Styles.Classic.BuffLoc = true end
local InitRan = false
...
function module:OnEnable()
	...
	if (SUI.DBMod.Artwork.Style == "Classic") then
		...
		if (not Artwork_Core:BartenderProfileCheck(SUI.DB.Styles.Classic.BartenderProfile,true)) then module:CreateProfile(); end
	...
	SUI.opt.args["Artwork"].args["ActionBar"] = { name = "ActionBar Settings", type = "group",desc = L["ActionBarConfDesc"],
		args = {
			...
			Allenable = {name = L["AllBarEnable"], type="toggle",order=2,
				get = function(info) return SUI.DB.ActionBars.Allenable; end,
				set = function(info,val) for i = 1,6 do SUI.DB.ActionBars["bar"..i].enable,DB.ActionBars.Allenable = val,val; end end
			},
			Allalpha = {name = L["AllBarAlpha"], type="range",order=2.1,width="double",
				min=0, max=100, step=1,
				get = function(info) return SUI.DB.ActionBars.Allalpha; end,
				set = function(info,val) for i = 1,6 do SUI.DB.ActionBars["bar"..i].alpha,DB.ActionBars.Allalpha = val,val; end end
			},
			Bar1 = { name = "Bar 1", type = "group", inline=true, args = {
				bar1alpha = {name = L["Alpha"], type="range",min=0, max=100, step=1, width="double",
					get = function(info) return SUI.DB.ActionBars.bar1.alpha; end,
					set = function(info,val) if SUI.DB.ActionBars.bar1.enable == true then SUI.DB.ActionBars.bar1.alpha = val end end
				},
				bar1enable = {name = L["Enabled"], type="toggle",
					get = function(info) return SUI.DB.ActionBars.bar1.enable; end,
					set = function(info,val) SUI.DB.ActionBars.bar1.enable = val end
				},
			}},
			Bar2 = { name = "Bar 2", type = "group", inline=true, args = {
				bar2alpha = {name = L["Alpha"], type="range",min=0, max=100, step=1, width="double",
					get = function(info) return SUI.DB.ActionBars.bar2.alpha; end,
					set = function(info,val) if SUI.DB.ActionBars.bar2.enable == true then SUI.DB.ActionBars.bar2.alpha = val end end
				},
				bar2enable = {name = L["Enabled"], type="toggle",
				get = function(info) return SUI.DB.ActionBars.bar2.enable; end,
					set = function(info,val) SUI.DB.ActionBars.bar2.enable = val end
				}
			}},
			Bar3 = { name = "Bar 3", type = "group", inline=true, args = {
				bar3alpha = {name = L["Alpha"], type="range",min=0, max=100, step=1, width="double",
					get = function(info) return SUI.DB.ActionBars.bar3.alpha; end,
					set = function(info,val) if SUI.DB.ActionBars.bar3.enable == true then SUI.DB.ActionBars.bar3.alpha = val end end
				},
				bar3enable = {name = L["Enabled"], type="toggle",
					get = function(info) return SUI.DB.ActionBars.bar3.enable; end,
					set = function(info,val) SUI.DB.ActionBars.bar3.enable = val end
				}
			}},
			Bar4 = { name = "Bar 4", type = "group", inline=true, args = {
				bar4alpha = {name = L["Alpha"], type="range",min=0, max=100, step=1, width="double",
					get = function(info) return SUI.DB.ActionBars.bar4.alpha; end,
					set = function(info,val) if SUI.DB.ActionBars.bar4.enable == true then SUI.DB.ActionBars.bar4.alpha = val end end
				},
				bar4enable = {name = L["Enabled"], type="toggle",
					get = function(info) return SUI.DB.ActionBars.bar4.enable; end,
					set = function(info,val) SUI.DB.ActionBars.bar4.enable = val end
				}
			}},
			Bar5 = { name = "Bar 5", type = "group", inline=true, args = {
				bar5alpha = {name = L["Alpha"], type="range",min=0, max=100, step=1, width="double",
					get = function(info) return SUI.DB.ActionBars.bar5.alpha; end,
					set = function(info,val) if SUI.DB.ActionBars.bar5.enable == true then SUI.DB.ActionBars.bar5.alpha = val end end
				},
				bar5enable = {name = L["Enabled"], type="toggle",
					get = function(info) return SUI.DB.ActionBars.bar5.enable; end,
					set = function(info,val) SUI.DB.ActionBars.bar5.enable = val end
				}
			}},
			Bar6 = { name = "Bar 6", type = "group", inline=true, args = {
				bar6alpha = {name = L["Alpha"], type="range",min=0, max=100, step=1, width="double",
					get = function(info) return SUI.DB.ActionBars.bar6.alpha; end,
					set = function(info,val) if SUI.DB.ActionBars.bar6.enable == true then SUI.DB.ActionBars.bar6.alpha = val end end
				},
				bar6enable = {name = L["Enabled"], type="toggle",
					get = function(info) return SUI.DB.ActionBars.bar6.enable; end,
					set = function(info,val) SUI.DB.ActionBars.bar6.enable = val end
				}
			}},
		}
	};
	SUI.opt.args["Artwork"].args["popup"] = { name = L["PopupAnimConf"], type = "group",
		...
		args = {
			popup1anim = {	name = L["LPopupAnimate"], type="toggle", order=1, width="full",
				get = function(info) return SUI.DB.ActionBars.popup1.anim; end,
				set = function(info,val) SUI.DB.ActionBars.popup1.anim = val; end
			},
			popup1alpha = {	name = L["LPopupAlpha"], type="range", order=2,
				min=0, max=100, step=1,
				get = function(info) return SUI.DB.ActionBars.popup1.alpha; end,
				set = function(info,val) if SUI.DB.ActionBars.popup1.enable == true then SUI.DB.ActionBars.popup1.alpha = val end end
			},
			popup1enable = {name = L["LPopupEnable"], type="toggle", order=3,
				get = function(info) return SUI.DB.ActionBars.popup1.enable; end,
				set = function(info,val) SUI.DB.ActionBars.popup1.enable = val end
			},
			popup2anim = {	name = L["RPopupAnimate"], type="toggle", order=4, width="full",
				get = function(info) return SUI.DB.ActionBars.popup2.anim; end,
				set = function(info,val) SUI.DB.ActionBars.popup2.anim = val; end
			},
			popup2alpha = {	name = L["RPopupAlpha"], type="range", order=5,
				min=0, max=100, step=1,
				get = function(info) return SUI.DB.ActionBars.popup2.alpha; end,
				set = function(info,val) if SUI.DB.ActionBars.popup2.enable == true then SUI.DB.ActionBars.popup2.alpha = val end end
			},
			popup2enable = {name = L["RPopupEnable"], type="toggle", order=6,
				get = function(info) return SUI.DB.ActionBars.popup2.enable; end,
				set = function(info,val) SUI.DB.ActionBars.popup2.enable = val end
			}
		}
	};
	SUI.opt.args["Artwork"].args["Artwork"] = {name = L["ArtworkOpt"],type="group",order=10,
		args = {
			Color = {name=L["ArtColor"],type="color",hasAlpha=true,order=.5,
				get = function(info) if not SUI.DB.Styles.Classic.Color.Art then return {1,1,1,1} end return unpack(SUI.DB.Styles.Classic.Color.Art) end,
				set = function(info,r,b,g,a) SUI.DB.Styles.Classic.Color.Art = {r,b,g,a}; module:SetColor(); end
			},
			...
			alpha = {name=L["Transparency"],type="range",order=1,width="full",
				...
				set = function(info,val) SUI.DB.alpha = (val/100); module:updateSpartanAlpha(); module:AddNotice(); end
			},
			TransparencyNotice = {
			...
			offsetauto = {name = L["AutoOffset"],type = "toggle",desc = L["AutoOffsetDesc"],order=3.1,
				...
				set = function(info,val) SUI.DB.yoffsetAuto = val end,
			}
		},
	}
