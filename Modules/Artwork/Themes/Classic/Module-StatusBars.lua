...
function module:EnableStatusBars()
	...
	local showAPTooltip = function(self)
		if HasArtifactEquipped() and not C_ArtifactUI.IsEquippedArtifactMaxed() then
			...
			SUI_StatusBarTooltipText:SetFormattedText(
				'( %s / %s ) %d%%',
				SUI:comma_value(xp),
				SUI:comma_value(xpForNextPoint),
				ratio * 100
	...
	SUI_StatusBar_Left:SetScript(
		...
		function(self)
			...
			if SUI.DB.StatusBars.left == 'az' and SUI.DB.StatusBars.AzeriteBar.ToolTip == 'hover' then
	...
	SUI_StatusBar_Right:SetScript(
		...
		function(self)
			...
			if SUI.DB.StatusBars.right == 'az' and SUI.DB.StatusBars.APBar.ToolTip == 'hover' then
				showAzeriteTooltip(self)
			end
		end
	)
	SUI_StatusBar_Right:SetScript(
		...
		function(self)
			...
			if SUI.DB.StatusBars.right == 'az' and SUI.DB.StatusBars.APBar.ToolTip == 'click' then
				showAzeriteTooltip(self)
			end
		end
	)
...
function module:UpdateStatusBars()
	if SUI.DB.StatusBars.left ~= 'disabled' then
		SUI_StatusBar_Left:Show()
		updateText(SUI_StatusBar_Left, 'left')
	else
		SUI_StatusBar_Left:Hide()
	end
	if SUI.DB.StatusBars.right ~= 'disabled' then
		SUI_StatusBar_Right:Show()
		updateText(SUI_StatusBar_Right, 'left')
	else
		SUI_StatusBar_Right:Hide()
	end
end
