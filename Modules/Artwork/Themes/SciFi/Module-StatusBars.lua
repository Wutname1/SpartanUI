...
function module:SetXPColors()
	...
	SciFi_ExperienceBarFill:SetVertexColor	(r,g,b,a);
	SciFi_ExperienceBarFillGlow:SetVertexColor(r,g,b,(a-.2));
	...
	SciFi_ExperienceBarLead:SetVertexColor	(r,g,b,a);
	SciFi_ExperienceBarLeadGlow:SetVertexColor(r,g,b,(a+.1));
...
function module:EnableStatusBars()
	do -- create the tooltip
		tooltip = CreateFrame("Frame","SciFi_StatusBarTooltip",SciFi_SpartanUI,"SciFi_StatusBars_TooltipTemplate");
		SciFi_StatusBarTooltipHeader:SetJustifyH("LEFT");
		SciFi_StatusBarTooltipText:SetJustifyH("LEFT");
		SciFi_StatusBarTooltipText:SetJustifyV("TOP");
		spartan:FormatFont(SciFi_StatusBarTooltipHeader, 12, "Core")
		spartan:FormatFont(SciFi_StatusBarTooltipText, 10, "Core")
	end
	do -- experience bar
		...
		xpframe:SetScript("OnEvent",function()
			...
			if now == 0 then
				SciFi_ExperienceBarFill:SetWidth(0.1);
				SciFi_ExperienceBarFillGlow:SetWidth(.1);
				SciFi_ExperienceBarLead:SetWidth(0.1);
			else
				SciFi_ExperienceBarFill:SetWidth((now/goal)*400);
				rested = (rested/goal)*400;
				if (rested+SciFi_ExperienceBarFill:GetWidth()) > 399 then rested = 400-SciFi_ExperienceBarFill:GetWidth(); end
				if rested == 0 then rested = .001 end
				SciFi_ExperienceBarLead:SetWidth(rested);
			end
		...
		local showXPTooltip = function()
			...
			SciFi_StatusBarTooltipHeader:SetText(a..b); -- Level 99 (9999 / 9999) 100% Experience
			local rested,text = GetXPExhaustion() or 0;
			if (rested > 0) then
				text = format(xptip1,format(xprest,(rested/UnitXPMax("player"))*100),200);
				SciFi_StatusBarTooltipText:SetText(text); -- Rested (15%) - 200% of normal experience gained from monsters.
			else
				SciFi_StatusBarTooltipText:SetText(format(xptip1,EXHAUST_TOOLTIP2,100)); -- You should rest at an Inn. 100% of normal experience gained from monsters.
			end
			tooltip:Show();
		end
