...
local SetBarColor = function(self, side)
	...
	if module.DB[side].AutoColor then
		...
		elseif display == 'rep' then
			...
			color1 = FACTION_BAR_COLORS[watchedFactionData and watchedFactionData.reaction or 7] or FACTION_BAR_COLORS[7]
		end
	end
...
local updateText = function(self)
	...
	elseif module.DB[side].display == 'rep' then
		local watchedFactionData = C_Reputation.GetWatchedFactionData()
		local name = watchedFactionData and watchedFactionData.name
		local low = watchedFactionData and watchedFactionData.currentReactionThreshold or 0
		local high = watchedFactionData and watchedFactionData.nextReactionThreshold or 0
		local current = watchedFactionData and watchedFactionData.currentStanding or 0
		local factionID = watchedFactionData and watchedFactionData.factionID
...
local showRepTooltip = function(self)
	local watchedFactionData = C_Reputation.GetWatchedFactionData()
	local name = watchedFactionData and watchedFactionData.name
	local react = watchedFactionData and watchedFactionData.reaction or 1
	local low = watchedFactionData and watchedFactionData.currentReactionThreshold or 0
	local high = watchedFactionData and watchedFactionData.nextReactionThreshold or 0
	local current = watchedFactionData and watchedFactionData.currentStanding or 0
	local repLevelLow = (current - low)
