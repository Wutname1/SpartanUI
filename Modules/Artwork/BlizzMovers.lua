...
local function AbilityBars()
	...
	if not extraActionEnabled then RestoreOriginalPosition('ExtraActionBar') end
	if not zoneAbilityEnabled and ZoneAbilityFrame then RestoreOriginalPosition('ZoneAbility') end

	if not extraActionEnabled and not zoneAbilityEnabled then return end

	-- Cache original positions before moving
	if extraActionEnabled then CacheOriginalPosition('ExtraActionBar', ExtraActionBarFrame) end
	if zoneAbilityEnabled and ZoneAbilityFrame then CacheOriginalPosition('ZoneAbility', ZoneAbilityFrame) end
	...
	if ExtraAbilityContainer then hooksecurefunc(ExtraAbilityContainer, 'AddFrame', function()
		ExtraAB.Reparent()
	end) end

	if ZoneAbilityFrame then hooksecurefunc(ZoneAbilityFrame, 'SetParent', function(_, parent)
...
local function AlertFrame()
	...
	if not SUI.DB.Artwork.BlizzMoverStates[moverName].enabled then
		RestoreOriginalPosition(moverName)
		if groupLootContainer and module.BlizzMoverCache[moverName .. '_GroupLoot'] then RestoreOriginalPosition(moverName .. '_GroupLoot') end
		return
	end

	-- Cache original positions before moving
	CacheOriginalPosition(moverName, alertFrame)
	if groupLootContainer then CacheOriginalPosition(moverName .. '_GroupLoot', groupLootContainer) end

	local holder = GenerateHolder(moverName)
	holder:SetSize(180, 40)

	AttachToHolder(alertFrame, holder, 'BOTTOM')
	if groupLootContainer then AttachToHolder(groupLootContainer, holder, 'BOTTOM') end

	hooksecurefunc(alertFrame, 'SetPoint', ResetPosition)
	if groupLootContainer then hooksecurefunc(groupLootContainer, 'SetPoint', ResetPosition) end
...
local function WidgetPowerBarContainer()
	...
	if not SUI.DB.Artwork.BlizzMoverStates[moverName].enabled then
		RestoreOriginalPosition(moverName)
		if playerPowerBarAlt and module.BlizzMoverCache[moverName .. '_PowerBarAlt'] then RestoreOriginalPosition(moverName .. '_PowerBarAlt') end
		return
	end

	-- Cache original positions before moving
	if widgetFrame then CacheOriginalPosition(moverName, widgetFrame) end
	if playerPowerBarAlt and not C_AddOns.IsAddOnLoaded('SimplePowerBar') then CacheOriginalPosition(moverName .. '_PowerBarAlt', playerPowerBarAlt) end
