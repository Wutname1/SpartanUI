...
function addon:OnInitialize()
	...
	SUI.opt.args['ModSetting'].args['FilmEffects'] = {
		...
		args = {
			enable = {
				...
				get = function(info)
					updateopts()
					return SUI.DBMod.FilmEffects.enable
				end,
				set = function(info, val)
					...
					SUI.DBMod.FilmEffects.enable = val
	...
	for k, v in ipairs(EffectList) do
		...
		SUI.opt.args['ModSetting'].args['FilmEffects'].args[v .. 'always'] = {
			...
			order = k + 1.2,
			get = function(info)
				return SUI.DBMod.FilmEffects.Effects[v].always
			end,
			set = function(info, val)
				if InCombatLockdown() then
					SUI:Print(L['Please leave combat first.'])
				...
				SUI.DBMod.FilmEffects.Effects[v].always = val
				FilmEffectEvent(nil, nil, nil)
			end
		}
		SUI.opt.args['ModSetting'].args['FilmEffects'].args[v .. 'AFK'] = {
			...
			get = function(info)
				...
				return SUI.DBMod.FilmEffects.Effects[v].afk
			end,
			set = function(info, val)
				SUI.DBMod.FilmEffects.Effects[v].afk = val
			end
		}
	end
end
...
function addon:OnEnable()
	...
	Container.vignette:SetTexture('Interface\\AddOns\\SpartanUI_FilmEffects\\images\\vignette')
	...
	Container.blur.layer1:SetTexture('Interface\\AddOns\\SpartanUI_FilmEffects\\images\\25ASA_Add')
	Container.blur.layer2:SetTexture('Interface\\AddOns\\SpartanUI_FilmEffects\\images\\25ASA_Mod')
	...
	while i <= iLimit do
		...
		Container.crisp[nameAdd]:SetTexture('Interface\\AddOns\\SpartanUI_FilmEffects\\images\\25ASA_Add')
		Container.crisp[nameMod]:SetTexture('Interface\\AddOns\\SpartanUI_FilmEffects\\images\\25ASA_Mod')
