...
function LDBAdapter:IsValidLDBObject(dataObject)
    ...
    if not dataObject.text and not dataObject.label then
        return false
    end
    
    -- Must be a data source
    if dataObject.type ~= "data source" then
        return false
    end
...
function LDBPluginWrapper:GetText()
    ...
    -- Handle suffix
    if self.ldbObject.suffix then
        text = (text or "") .. " " .. self.ldbObject.suffix
    end
...
function LDBPluginWrapper:UpdateTooltip(tooltip)
    ...
    if self.ldbObject.OnTooltipShow and type(self.ldbObject.OnTooltipShow) == "function" then
        ...
        if self.ldbObject.tooltiptext then
            tooltip:AddLine(self.ldbObject.tooltiptext, 1, 1, 1, true)
        end
    end
end
...
function LDBPluginWrapper:OnClick(button)
    ...
    if self.ldbObject.OnClick and type(self.ldbObject.OnClick) == "function" then
        ...
        if not success then
            LibsDataBar:DebugLog("error", "LDB OnClick error for " .. self.name .. ": " .. tostring(result))
        end
    end
end
    ...
    return {
        ...
        args = {
            ...
            enabled = {
                ...
                get = function() return LibsDataBar.config:GetPluginConfig(self.plugin.id, "enabled") end,
            ...
            showIcon = {
                ...
                disabled = function() return not self.ldbObject.icon end,
                get = function() return LibsDataBar.config:GetPluginConfig(self.plugin.id, "showIcon") end,
                set = function(_, value) LibsDataBar.config:SetPluginConfig(self.plugin.id, "showIcon", value) end,
                order = 4
            },
            ...
            showText = {
                ...
                get = function() return LibsDataBar.config:GetPluginConfig(self.plugin.id, "showText") end,
                set = function(_, value) LibsDataBar.config:SetPluginConfig(self.plugin.id, "showText", value) end,
                order = 5
            }
        }
    }
...
function LDBAdapter:OnLDBObjectChanged(name, key, value, dataObject)
    ...
    if key == "text" or key == "label" or key == "suffix" then
        ...
        if LibsDataBar.callbacks then
            LibsDataBar.callbacks:Fire("LibsDataBar_PluginTextChanged", wrapper.plugin.id, wrapper:GetText())
        end
    elseif key == "icon" then
        -- Icon changed - trigger button update
        if LibsDataBar.callbacks then
            LibsDataBar.callbacks:Fire("LibsDataBar_PluginIconChanged", wrapper.plugin.id, value)
        end
    end
...
function LDBAdapter:Cleanup()
    ...
    if self.callbacks.OnDataObjectCreated then
        LDB.UnregisterCallback(self, "LibDataBroker_DataObjectCreated")
    end
    
    if self.callbacks.OnDataObjectChanged then
