...
function EventManager:RegisterEvent(event, callback, options)
    ...
    if not self.eventRegistry[event] then
        ...
        local gameEvents = {
            ...
            "PLAYER_LEAVING_WORLD", "ZONE_CHANGED", "ZONE_CHANGED_NEW_AREA",
            "PLAYER_MONEY", "BAG_UPDATE", "UPDATE_BONUS_ACTIONBAR"
        }
...
function EventManager:FireEvent(event, ...)
    ...
    for callback, config in pairs(callbacks) do
        if config.enabled then
            tinsert(sortedCallbacks, {callback, config})
        end
    end
...
function EventManager:SafeCall(func, ...)
    ...
    if not success then
        lib:DebugLog("error", "Event callback failed: " .. tostring(result))
    end
...
function lib:RegisterBuiltinPlugins(bar)
    ...
    if self.ldb and self.ldb.autoDiscovery then
        C_Timer.After(1, function()
            ...
            if ldbCount > 0 then
                self:DebugLog("info", "Added " .. ldbCount .. " LDB plugins to main bar")
            end
        end)
    end
end
