...
function Container:Create(config)
    ...
    if LibsDataBar.callbacks then
        LibsDataBar.callbacks:Fire("LibsDataBar_ContainerCreated", config.id, container)
    end
...
function Container:SetupAnchoredBehavior()
    ...
    if self.config.anchorFrame then
        ...
        if anchorFrame then
            ...
            anchorFrame:HookScript("OnShow", function() self:UpdatePosition() end)
            anchorFrame:HookScript("OnHide", function() self:UpdatePosition() end)
        end
    end
end
...
function Container:StartDrag()
    ...
    -- Show snap indicator
    if self.config.behavior.snapToEdges or self.config.behavior.snapToOthers then
        self:ShowSnapZones()
    end
...
function Container:StopDrag()
    ...
    if snapPosition then
        self:SnapToPosition(snapPosition)
    end
...
function Container:SnapToPosition(position)
    ...
    if position.zone then
        LibsDataBar:DebugLog("info", "Container " .. self.id .. " snapped to " .. (position.zone.name or position.zone.type))
    end
end
...
function Container:AutoResize()
    ...
    if visiblePlugins > 1 then
        requiredWidth = requiredWidth + (spacing * (visiblePlugins - 1))
    end
...
function Container:Destroy()
    ...
    if LibsDataBar.callbacks then
        LibsDataBar.callbacks:Fire("LibsDataBar_ContainerDestroyed", self.id)
    end
