...
function ThemeManager:RegisterTheme(theme)
    ...
    if LibsDataBar.callbacks then
        LibsDataBar.callbacks:Fire("LibsDataBar_ThemeRegistered", theme.id, theme)
    end
...
function ThemeManager:SetCurrentTheme(themeId)
    ...
    if LibsDataBar.callbacks then
        LibsDataBar.callbacks:Fire("LibsDataBar_ThemeChanged", themeId, previousTheme)
    end
...
function ThemeManager:CreateCustomTheme(baseThemeId, newThemeId, modifications)
    ...
    -- Apply modifications
    if modifications.bar then
        self:MergeTable(customTheme.bar, modifications.bar)
    end
    
    if modifications.button then
        self:MergeTable(customTheme.button, modifications.button)
    end
    
    if modifications.tooltip then
        self:MergeTable(customTheme.tooltip, modifications.tooltip)
    end
...
function ThemeManager:RemoveCustomTheme(themeId)
    ...
    if self.currentTheme == themeId then
        self:SetCurrentTheme(self.defaultTheme)
    end
