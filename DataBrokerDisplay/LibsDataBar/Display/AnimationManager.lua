...
function AnimationManager:Animate(target, preset, options)
    ...
    if options then
        config = self:MergeConfig(config, options)
    end
...
function AnimationManager:StopAnimation(animationId, complete)
    ...
    if animation.onComplete then
        animation.onComplete(animation.target, complete)
    end
...
function AnimationManager:Update()
    ...
    for animationId, animation in pairs(self.animations) do
        ...
        for prop, propConfig in pairs(animation.properties) do
            ...
            if propConfig.relative then
                currentValue = startValue + (propConfig.from or 0) + ((propConfig.to or 0) - (propConfig.from or 0)) * easedProgress
            end
...
function AnimationManager:StopAllAnimations(frame)
    local toStop = {}
    for animationId, animation in pairs(self.animations) do
        if animation.target == frame then
            tinsert(toStop, animationId)
        end
    end
...
if LibsDataBar.animations then
    LibsDataBar.animations:Initialize()
end
