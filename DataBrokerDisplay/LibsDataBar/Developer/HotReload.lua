...
function HotReload:StartWatching()
    ...
    if not self.watchTimer then
        self.watchTimer = C_Timer.NewTicker(2, function()
...
function HotReload:QueueReload(filePath)
    ...
    if not self.reloadTimer then
        self.reloadTimer = C_Timer.NewTimer(0.5, function()
...
function HotReload:ReloadPlugin(pluginName)
    ...
    for barId, bar in pairs(LibsDataBar.bars or {}) do
        if bar.plugins and bar.plugins[plugin.id] then
            bar:RemovePlugin(plugin.id)
        end
    end
    
    -- Re-initialize plugin
    if plugin.OnInitialize then
        pcall(plugin.OnInitialize, plugin)
    end
...
function HotReload:ReloadThemes()
    ...
    if LibsDataBar.themes then
        ...
        if currentTheme then
            LibsDataBar.themes:SetCurrentTheme(currentTheme)
        end
    end
...
function HotReload:PostReloadCleanup()
    ...
    if LibsDataBar.validator then
        for pluginId, plugin in pairs(LibsDataBar.plugins or {}) do
            if plugin then
                LibsDataBar.validator:ValidatePlugin(plugin)
            end
        end
    end
...
function HotReload:CreateTestEnvironment()
    ...
    for pluginId, plugin in pairs(LibsDataBar.plugins or {}) do
        if plugin and not testBar.plugins[pluginId] then
            testBar:AddPlugin(plugin)
        end
    end
