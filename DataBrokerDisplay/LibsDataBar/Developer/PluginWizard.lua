...
local WIZARD_STEPS = {
    {
        ...
        fields = {
            ...
            { name = "category", label = "Category", type = "dropdown", required = true, values = {
    ...
    },
    {
        ...
        fields = {
            { name = "pluginType", label = "Plugin Type", type = "radio", required = true, values = {
    ...
    },
    {
        ...
        fields = {
            { name = "iconPath", label = "Icon Path", type = "editbox", 
              default = "Interface\\Icons\\INV_Misc_QuestionMark", 
              placeholder = "Interface\\Icons\\INV_Misc_QuestionMark" },
            { name = "updateInterval", label = "Update Interval (seconds)", type = "slider", 
              min = 0.1, max = 10, step = 0.1, default = 1.0 },
            { name = "defaultText", label = "Default Display Text", type = "editbox", 
              default = "Plugin Text", placeholder = "Text to display" }
        }
    },
    {
        id = "review",
...
function PluginWizard:CreateStepFields(step)
    for _, field in ipairs(step.fields) do
        ...
        if widget then
            self.wizardFrame:AddChild(widget)
        end
    end
end
...
function PluginWizard:CreateFieldWidget(field)
    ...
    if field.type == "editbox" then
        ...
        if field.placeholder then
            widget:SetLabel(field.label .. " (" .. field.placeholder .. ")")
        end
            ...
            radio:SetCallback("OnValueChanged", function(w, event, checked)
                if checked then
                    ...
                    for _, child in ipairs(group.children) do
                        if child ~= radio and child.SetValue then
    ...
    for _, field in ipairs(step.fields) do
        if field.required then
            ...
            if not value or value == "" then
                table.insert(errors, field.label .. " is required")
            end
        end
    end
