...
local function AddToUnitFrameOptions(frameName)
    local UF = SUI.UF
    if not UF or not UF.Options then return end
    ...
    local backgroundBorderOptions = BackgroundBorder:GenerateCompleteOptions(
        'UnitFrame_' .. frameName,
        getSettings,
        setSettings, 
        updateDisplay
    )
...
local function MigrateArtworkSystem()
    ...
    if not Artwork or not Artwork.BarBG then return end
